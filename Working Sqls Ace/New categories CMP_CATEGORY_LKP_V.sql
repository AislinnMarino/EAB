...
WHERE
    ...
    SELECT DISTINCT
    ('PELL'| |A.AID_YEAR) As Category_id,
           ('PELL ELIG ' | |A.AID_YEAR) 
               AS Category_Desc,
           'UB'
               AS GROUP_ID
      FROM ps_rpt.PS_ISIR_CONTROL_V A
     WHERE   A.aid_year >='2011';

        select * from    ps_rpt.PS_ISIR_CONTROL;
...
WHERE
    pe.SOURCEKEY >= '2111';
          ...
          /* Formatted on 1/7/2026 9:35:31 AM (QP5 v5.401) */
CREATE OR REPLACE FORCE VIEW PS_RPT.CMP_CATEGORY_LKP_V
(
    CATEGORY_ID,
    CATEGORY_DESC,
    GROUP_ID
...
WHERE
    pe.SOURCEKEY >= '2111'
    ORDER BY Category_Desc;
GRANT SELECT ON PS_RPT.CMP_CATEGORY_LKP_V TO MSA_EAB;
GRANT SELECT ON PS_RPT.CMP_CATEGORY_LKP_V TO MSA_EAB;