WITH
        ...
        AS
            (SELECT DISTINCT
                    EMPB.PERSON_NUMBER AS STAFF_ID, PNT.PRINCIPAL AS SSO_ID
               FROM UBS_EMP.EMPLOYEE_BASICS_V  EMPB
                    LEFT JOIN DCE.PERSON_NUMBER_T PNT
                        ON     PNT.PERSON_NUMBER = EMPB.PERSON_NUMBER
                           AND PNT.STATUS = 'A'),
        ...
        AS
            (SELECT p.emplid
               FROM PS_RPT.ps_PERSON p
              WHERE     p.DT_OF_DEATH IS NOT NULL
                    AND p.emplid IN (SELECT student_id FROM ActiveStu
                                     UNION ALL
                                     SELECT staff_id FROM ActiveStaff
                                     UNION ALL
                                     SELECT staff_id FROM ExtendedStaff)),
        EntirePop
        AS
            (SELECT ActiveStu.STUDENT_ID    AS STAFF_ID,
                    CASE
                        WHEN (   ActiveStu.STUDENT_ID IS NULL
                              OR DeceasedPop.emplid IS NOT NULL)
                        THEN
                            'N'
                        ELSE
                            'A'
               ...
               FROM DCE.PERSON_NUMBER_T  PNT
                    ...
                    LEFT JOIN DeceasedPop
                        ON DeceasedPop.emplid = PNT.PERSON_NUMBER
             UNION
             SELECT ExtendedStaff.STAFF_ID,
                    CASE
                        WHEN (   ActiveStaff.STAFF_ID IS NULL
                              OR DeceasedPop.emplid IS NOT NULL)
                        THEN
                            'N'
                        ELSE
                    ...
                    ' '    AS PIN,
                    CASE
                        WHEN     ActiveStaff.SSO_ID IS NULL
                             AND ExtendedStaff.SSO_ID IS NULL
                        THEN
                            ' '
                        ELSE
                            COALESCE (ActiveStaff.SSO_ID,
                                      ExtendedStaff.SSO_ID)
                    END    AS SSO_ID,
                    ' '    AS ALTERNATE_USER_ID
               FROM ExtendedStaff
                    ...
                    LEFT JOIN DeceasedPop
                        ON DeceasedPop.emplid = ExtendedStaff.STAFF_ID)
    SELECT STAFF_ID,
           ACTIVE_FLAG,
           CAN_LOGIN,
      ...
      FROM (SELECT STAFF_ID,
                   ...
                   DENSE_RANK ()
                       OVER (PARTITION BY STAFF_ID ORDER BY ACTIVE_FLAG)    AS SEQ_NO
              FROM EntirePop)
     WHERE SEQ_NO = 1
    UNION
